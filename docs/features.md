...
| 内嵌终端 | `/terminal` 命令在 launcher 内联展开终端：基于 **libvterm**（VT 解析/屏幕状态内核，vendored 纯 C）+ **ConPTY** 起 shell（pwsh > powershell > cmd）+ QPainter 逐格自绘 + 键盘编码。支持 ANSI 彩色、光标、窗口 resize 往返（reflow）、可交互（vim/htop 等）；**ANSI 16 色跟主题**（调色板注入 vterm，`ls --color`/vim 配色与当前主题一致，主题 JSON 可选 `ansi` 数组，缺失回退内置）；**鼠标上报**（应用开启鼠标模式时 vim/htop/tmux/lazygit 可点击/拖动/滚轮，按 Shift 强制本地选区）；**滚动回看**（5000 行历史，滚轮回看/`Shift+PageUp/PageDown` 键盘翻页/`Shift+Home/End` 跳顶贴底/按键贴底/备用屏滚轮转方向键/右侧位置指示条）；**鼠标选择与复制粘贴**（拖选跨历史行、双击选词按分隔符集合分词/路径域名整体选中、Ctrl+Shift+C/V、右键粘贴、bracketed paste）；**控制键**（`Ctrl+[`=Esc、`Ctrl+\ ] ^ _`、`Ctrl+Space`=NUL）；**中文 IME**（预编辑串绘制、候选框跟随光标）；关闭后可重开新会话。**终端栈已抽为可嵌入的 `TerminalPane`**（TerminalView+ConPty+PtyReader+会话接线），宿主为 launcher 主窗口：`/terminal` 在 launcher 内**内联展开终端模式**（卡片морф为终端，`Ctrl+\`` 进/出、Esc 保留给终端、失焦不隐藏、会话跨模式后台留存）。**多标签页**（`TerminalTabs` 容器）：`Ctrl+Shift+T` 新建 / `Ctrl+Shift+W` 关闭当前 / `Ctrl+Tab`·`Ctrl+Shift+Tab` 循环切换 / `Alt+1..9` 跳转 / “+”按钮（单标签时标签栏自动隐藏，回归干净单会话观感）；**标签名显示当前目录最后一层**（pwsh/powershell 启动时注入 prompt 钩子把窗口标题设为 cwd，包住 oh-my-posh 不破坏；cmd 无钩子回退 `Terminal N`），关闭按钮为主题化自绘 `×`；shell 退出自动关该标签，关到最后一个则退出终端模式；所有管理键与 `Ctrl+\`` 统一在容器层 event filter 拦截（抢在 TerminalView raw 送 PTY 之前）。**未做**：Linux 端（ConPTY→forkpty）、脏区局部重绘、字体/shell/scrollback 等配置化。工程坑见 [notes/embedded-terminal-libvterm.md](notes/embedded-terminal-libvterm.md)、[notes/terminal-inline-mode.md](notes/terminal-inline-mode.md) |
| 命令执行插件 | `:` 前缀触发 wrap 一次性展示（如 `: ls` Enter 在 launcher 卡片内展示输出，Esc 回搜索）；`Ctrl+Enter` 或 `::` 前缀（如 `:: vim`）进入内联终端并**新开标签**（vim/btop 等可并存、会话持久）。runner 由 `main.cpp` 装配；执行过的命令按 frecency 上浮 |
